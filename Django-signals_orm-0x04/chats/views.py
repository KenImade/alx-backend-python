...
def conversation_list(request):
    ...
    This view's output will be cached for 60 seconds. New data, even if 
    present in the database, will not be displayed until the cache expires.
    """
    current_user = request.user
    ...
    # Query the primary messages
    threads = Message.objects.filter(
        parent_message__isnull=True, 
        # Check if the user is the sender OR the receiver of the thread starter
    ...
    # Example of using the custom manager for the unread count (not cached by the decorator)
    unread_count = Message.unread.for_user(current_user).count()

    context = {
        'threads': threads,
        'unread_count': unread_count,
        'cached_at': timezone.now() # Useful for testing if the cache is working
    }
    return render(request, 'messaging/conversation_list.html', context)